...
class DMProduct(Product):
    ...
    def manipulate_tea_name(self, name):
        if "-Fenchel-Anis-Kümmel" in name:
            name = name
        name = self.remove_brand_decimal_comma_and_split(name)[0]
        name = self.clean_name_from_weight_tea_bag_data(name)

        # get type to remove it
        type_prefix = self.get_type_prefix(name)
        tea_type = self.get_final_type(type_prefix)

        # remove lose
        name = name.replace("lose", "")
        # remove demeter
        name = name.replace("demeter", "")
        # dm gives teas name with the type of the tee as a prefix
        # for teas named Krautertee, Früchteteemischung, Matcha Tee Grün gemahlen
        # was changed from
        #if name and type_prefix and tea_type:
            #name = type_prefix
        # to
        if name.strip() == type_prefix:
            name = type_prefix

        elif tea_type == "herbal" or tea_type == "fruit" or tea_type == "green":
            if "Milder" in type_prefix or "Wintertee" in type_prefix:
                name = name
            ...
            elif type_prefix == "gewürzt":
                name = remove_suffix(name, "gewürzt")

            else:
                name = f"Chai{name[len(type_prefix):]}"
    ...
    def get_final_type(self, type_prefix):
        herbal = ["Früchte- & Kräutertee Bio", "Kräutertee Bio", "Kräutertee-Mischung",
                  ...
                  "Kräutertee ", "Kräutertee", "Urorenal"]
    ...
    def get_type_prefix(self, name):
        ...
        substrings_to_return = ["Früchte-Gewürztee", "Babytee", "Früchte- & Kräutertee Bio",
                                ...
                                "Matcha Tee Grün gemahlen", "gewürzt"]
    ...
    def manipulate_ingredients(self, ingredient_text):
        ...
        ingredient_notes = ["aus kontrolliert biologischer Landwirtschaft 70% Rainforest Alliance-zertifiziert. Mehr dazu auf ra.org",
    ...
    def get_brand_page_url(self):
        self.brand_page_url = self.store_page_url if (
                self.get_brand == "dmBio" or
                self.get_brand == "Mivolis" or
                self.get_brand == "babylove") else None
...
if __name__ == "__main__":
    dm_scraper = DMScraper("https://www.dm.at/ernaehrung/kaffee-tee-kakao/tee?allCategories.id0=042202&pageSize0=30&sort0=editorial_relevance&currentPage0=", "dm")
    dm_scraper.run_multiple_pages('div[data-dmid="product-tile-container"]', 0)